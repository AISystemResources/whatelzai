...
function AwardBadge({ award }: { award: { title: string; track?: string } }) {
  return (
    <span
      className={`inline-flex items-center gap-1 rounded px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest ${awardColour(award.title)}`}
    >
      {award.title}
      {award.track ? ` · ${award.track}` : ""}
    </span>
  );
}
          ...
          {sorted.map((h) => (
            ...
            >
              <div>
                ...
                <p className="mt-1 font-medium text-zinc-900 leading-snug">
                  {h.name}
                </p>
                {h.project_name && (
                  <p className="text-xs text-zinc-500 mt-0.5">
                ...
                {h.organizer && (
                  <p className="text-xs text-zinc-400 mt-0.5">{h.organizer}</p>
                )}
              </div>
              {h.awards.length > 0 && (
                <div className="flex flex-wrap gap-1">
