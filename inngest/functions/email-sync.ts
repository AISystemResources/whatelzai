...
export const emailSync = inngest.createFunction(
  {
    id: "email-sync",
    name: "Email Sync",
    triggers: [{ cron: "TZ=Asia/Singapore */30 * * * *" }],
  },
  async ({ step }) => {
    const { data: cfg } = await supabaseAdmin
      .from("system_config")
      .select("value")
      .eq("key", "email_last_sync_at")
      .single();
    const sinceMs = cfg?.value
      ? new Date(cfg.value as string).getTime()
    ...
    const { data: userCfg } = await supabaseAdmin
      ...
      .single();
    if (!userCfg?.value)
      throw new Error("clerk_admin_user_id not set in system_config");

    const clerk = createClerkClient({
      secretKey: process.env.CLERK_SECRET_KEY,
    });
    const oauthTokens = await clerk.users.getUserOauthAccessToken(
      userCfg.value as string,
      "oauth_google",
    );
    const oauthToken = oauthTokens.data[0];
    if (!oauthToken?.token)
      throw new Error(
        "No Google OAuth token found for admin user — re-authenticate via Clerk",
      );

    const messages = await step.run("fetch-gmail", () =>
      listNewMessages(sinceMs, oauthToken.token),
    );
    ...
    for (const msg of messages) {
      ...
      const { companyId, applicationId } =
        msg.direction === "inbound"
          ? await step.run(`match-${msg.messageId}`, () =>
              matchEmailToApplication(msg.from),
            )
      ...
      if (applicationId && inserted) {
        ...
        if (newStatus) {
          await supabaseAdmin
            .from("applications")
            .update({
              response_status: emailClass,
              last_response_at: msg.receivedAt,
              status: newStatus,
            })
            .eq("id", applicationId);
        }
