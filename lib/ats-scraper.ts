...
export async function scrapeGreenhouse(
  ...
  return ((data.jobs ?? []) as Record<string, unknown>[]).map((j) => ({
    ...
    external_url:
      (j["absolute_url"] as string | undefined) ??
      `https://boards.greenhouse.io/${slug}/jobs/${j["id"]}`,
    company: companyName,
    role: (j["title"] as string | undefined) ?? "",
    location:
      (j["location"] as Record<string, string> | undefined)?.["name"] ?? null,
    remote_type: detectRemote(
      (j["location"] as Record<string, string> | undefined)?.["name"] ?? null,
    ),
...
export async function scrapeLever(
  ...
  return (Array.isArray(data) ? (data as Record<string, unknown>[]) : []).map(
    (j) => ({
      external_id: (j["id"] as string | undefined) ?? "",
      external_url:
        (j["hostedUrl"] as string | undefined) ??
        `https://jobs.lever.co/${slug}/${j["id"]}`,
      company: companyName,
      role: (j["text"] as string | undefined) ?? "",
      location:
        (j["categories"] as Record<string, string> | undefined)?.["location"] ??
        null,
      remote_type: detectRemote(
        (j["categories"] as Record<string, string> | undefined)?.[
          "commitment"
        ] ?? null,
      ),
...
export async function scrapeAshby(
  ...
  const res = await fetch(
    `https://api.ashbyhq.com/posting-api/job-board/${slug}`,
    {
      ...
      headers: {
        ...
        "User-Agent": "whatelz-job-hunter/1.0",
      },
      body: JSON.stringify({}),
    },
  );
  ...
  return (data.jobPostings ?? []).map((j) => ({
    ...
    description:
      (j["descriptionHtml"] as string | undefined)?.replace(/<[^>]+>/g, "") ??
      null,
...
export async function scrapeCompany(
  ...
  switch (atsType.toLowerCase()) {
    case "greenhouse":
      return scrapeGreenhouse(atsSlug, companyName);
    case "lever":
      return scrapeLever(atsSlug, companyName);
    case "ashby":
      return scrapeAshby(atsSlug, companyName);
    default:
      return [];
  }
}
