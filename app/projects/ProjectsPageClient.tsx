...
function StatusBadge({ status }: { status: ProjectStatus | null }) {
  ...
  return (
    <span
      className={`font-mono text-[10px] uppercase tracking-widest px-2 py-0.5 rounded ${STATUS_CLASSES[status]}`}
    >
      {status}
    </span>
  );
            ...
            >
              ...
              <div className="flex flex-wrap gap-1.5">
                {(p.tech_stack ?? []).map((s) => (
                  <span
                    key={s}
                    className="font-mono text-[10px] uppercase tracking-wide bg-zinc-100 text-zinc-600 px-2 py-0.5 rounded"
                  >
                    {s}
                  </span>
                ))}
