...
export async function renderCoverLetterPdf(
  ...
  const contactLine = [identity.email, identity.phone]
    .filter(Boolean)
    .join("  ·  ");
  ...
  const doc = React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(Text, { key: k(), style: S.contact }, contactLine),
      ),
      ...
      companyName
        ? React.createElement(
            ...
            {
              key: k(),
              style: { position: "absolute", bottom: 28, right: 52 },
            },
            React.createElement(
              Text,
              { style: { fontSize: 7.5, color: "#a1a1aa" } },
              companyName,
            ),
          )
