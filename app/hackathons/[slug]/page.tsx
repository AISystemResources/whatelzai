...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!h) return {};
  return { title: `${h.name} — ${s.owner_name}` };
}
...
export default async function HackathonDetailPage({ params }: Props) {
  ...
  const embedUrl =
    h.demo_url && isYouTube(h.demo_url) ? youtubeEmbedUrl(h.demo_url) : null;
    ...
    <section className="px-6 py-24 sm:px-8 sm:py-32">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mt-8 space-y-2">
          ...
          {h.organizer && <p className="text-zinc-500">{h.organizer}</p>}
        </div>
        ...
        {h.team.length > 0 && (
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            {h.team.map((member) => (
              <span
                key={member}
                className="font-mono text-xs text-zinc-600 bg-zinc-100 px-2 py-0.5 rounded"
              >
                {member}
              </span>
            ))}
        ...
        {posts && posts.length > 0 && (
          <div className="mt-16 border-t border-zinc-100 pt-10">
            <p className="font-mono text-[10px] uppercase tracking-widest text-zinc-400 mb-4">
              Related Posts
            </p>
