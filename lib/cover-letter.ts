...
const BLOCKCHAIN_RE =
  /blockchain|crypto(?!graphy)|web3|defi|nft|smart.?contract|ethereum|solidity|\btoken\b/i;
const AI_RE =
  /\bai\b|machine.?learning|\bml\b|\bllm\b|deep.?learning|neural.?network|\bnlp\b|computer.?vision|generative/i;

export function selectResumeVariant(
  jdText: string,
): "AI Engineer" | "Blockchain Engineer" | "Software Engineer" {
...
function buildSystemPrompt(ownerName: string, firstName: string): string {
  return `You write cover letters for ${ownerName}. Produce exactly four paragraphs, no headers, no subject line.

STRUCTURE:
Para 1 — Hook (2-3 sentences)
  - Who ${firstName} is in one line
...
Para 3 — Fit beyond the JD (2-3 sentences)
  - Something specific about the company's actual problem or domain that ${firstName} understands
  - Not generic skill-matching — show insight into why ${firstName} specifically fits
...
- Sign-off: just the name — ${ownerName}
...
- Dense, factual, low filler — ${firstName}'s voice matches ${firstName}'s resume: direct, no hype
...
export async function generateCoverLetter(
  ...
  const message = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `Write a cover letter for ${identity.owner_first_name} applying to: ${jobRole} at ${companyName}
