...
export async function lookupHackathonRoute(
  text: string,
): Promise<string | null> {
  ...
  if (
    /most.{0,10}recent|latest.{0,10}win|recent.{0,10}win|newest.{0,10}win/.test(
      t,
    )
  ) {
    ...
    const winner = (data ?? []).find(
      (h) => Array.isArray(h.awards) && h.awards.length > 0,
    );
...
export async function lookupCareerRoute(text: string): Promise<string | null> {
  ...
  if (
    /current.{0,20}(intern|job|role|position|work)|his.{0,10}current/.test(t)
  ) {
...
export function detectTopic(text: string): string | null {
  const t = text.toLowerCase();
  if (
    /hackathon|hackomania|pan.?sea|singhack|coding.{0,20}win|win.{0,20}hack/.test(
      t,
    )
  )
    return "hackathons";
  if (
    /internship|career|prudential|setel|asiaverify|work.{0,15}experience|experience.{0,15}work/.test(
      t,
    )
  )
    return "career";
  ...
  if (
    /leadership|student.{0,10}council|club.{0,10}(president|vice|secretary|chair)|organisation.{0,10}role/.test(
      t,
    )
  )
    return "leadership";
  if (/mentorship|mentor|mentee|programme|gftn|paypal.{0,10}mentor/.test(t))
    return "mentorship";
  if (/contact|email.{0,15}edmund|reach.{0,15}edmund|get.{0,15}touch/.test(t))
    return "contact";
  if (/youtube|instagram|\bmedium\b|linkedin|channel/.test(t))
    return "channels";
  return null;
}
...
export function buildSteps(
  ...
  if (nav.mode === "section") {
    ...
    const dataAttr =
      nav.target === "hackathons" ? "data-hackathon-slug" : "data-career-slug";
