...
interface Props {
  readonly initialCompanies: Company[];
}
...
export function CompanyList({ initialCompanies }: Props) {
  ...
  async function handleAdd() {
    ...
    if (data.company) {
      setCompanies((prev) =>
        [...prev, data.company!].sort((a, b) => a.priority - b.priority),
      );
      setAdding(false);
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-sm text-zinc-500">
          {companies.length} companies on watchlist
        </p>
        <button
        ...
        <div className="border border-zinc-200 rounded p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <select
              value={form.ats_type}
              onChange={(e) =>
                setForm((f) => ({ ...f, ats_type: e.target.value }))
              }
              className={inputCls}
            >
              {ATS_TYPES.map((t) => (
            ...
            <input
              ...
              value={form.ats_slug}
              onChange={(e) =>
                setForm((f) => ({ ...f, ats_slug: e.target.value }))
              }
              className={inputCls}
            />
            <select
          ...
          <div key={c.id} className="flex items-center justify-between py-3">
            <div>
              ...
              <p className="text-xs text-zinc-400">
                {c.industry ?? "—"} · {c.ats_type ?? "no ATS"}
                {c.ats_slug ? ` / ${c.ats_slug}` : ""} · P{c.priority}
              </p>
            </div>
            <button
