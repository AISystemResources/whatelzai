...
export const jobDiscovery = inngest.createFunction(
  {
    id: "job-discovery",
    name: "Job Discovery",
    triggers: [{ cron: "TZ=Asia/Singapore 0 6,12,18 * * *" }],
  },
  async ({ step }) => {
    ...
    for (const company of companies) {
      ...
      const rejectedCount = toInsert.filter(
        (r) => r.status === "rejected_by_user",
      ).length;
      ...
      totalDuplicates +=
        raw.length -
        rejectedCount -
        insertedRows.filter((r) => r.status === "new").length;

      const newRows = insertedRows.filter(
