...
async function migrateJobSources(): Promise<void> {
  ...
  // Fetch existing rows already seeded by DDL migration — map name → uuid
  ...
  for (const r of rows) {
    if (existingByName.has(r.name)) {
      // Reuse the existing UUID so FKs from job_listings resolve correctly
      jobSourceMap.set(r.id, existingByName.get(r.name)!);
    } else {
      const id = randomUUID();
      ...
      toInsert.push({
        ...
        is_enabled: toBool(r.is_enabled),
  ...
  if (toInsert.length > 0) await insert("job_sources", toInsert);
  else
    console.log(
      `  ✓ job_sources: all ${rows.length} rows already seeded — IDs mapped`,
    );
}
...
async function migrateCompanies(): Promise<void> {
  ...
  const mapped = rows.map((r) => {
    ...
    return {
      ...
      last_fetch_count: r.last_fetch_count ?? 0,
      notes: r.notes,
      created_at: toTs(r.created_at) ?? new Date().toISOString(),
...
async function migrateEmailAccounts(): Promise<void> {
  ...
  const mapped = rows.map((r) => {
    ...
    return {
      ...
      is_active: toBool(r.is_active),
      last_sync_at: toTs(r.last_sync_at),
      created_at: toTs(r.created_at) ?? new Date().toISOString(),
    };
  });
...
async function migrateApplicationEvents(): Promise<void> {
  ...
  const rows = sqlite
    .prepare("SELECT * FROM application_events")
    .all() as any[];
  const mapped = rows
    .map((r) => ({
      id: randomUUID(),
...
async function migrateEmails(): Promise<void> {
  ...
  const mapped = rows.map((r) => ({
    ...
    application_id: r.application_id
      ? (appMap.get(r.application_id) ?? null)
      : null,
    ...
    is_flagged: toBool(r.is_flagged),
  ...
  if (tableExists("writing_samples")) {
    ...
    await insert(
      ...
      rows.map((r) => ({
        ...
        category: r.category,
        content: r.content,
        was_edited: toBool(r.was_edited),
        original: r.original,
        created_at: toTs(r.created_at) ?? new Date().toISOString(),
      })),
    );
