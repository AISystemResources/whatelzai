...
// Client wrapper for the template start action. Same shape as the base
// /feedback StartForm but posts the template slug alongside the email.
export function TemplateStartForm({ slug }: { slug: string }) {
  const [state, action, pending] = useActionState(startFromTemplate, null);
  const inputCls =
    ...
    <form action={action} className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-2 font-mono text-[10px] text-zinc-400">
          Kept private. Used so you can come back to finish later, and so I can
          reply. Never shown publicly.
        </p>
      </div>
