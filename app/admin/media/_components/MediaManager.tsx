...
export function MediaManager({ assets: initial, onSave, onDelete }: Props) {
  ...
  async function remove() {
    if (!selected || !confirm("Delete this image? This cannot be undone."))
      return;
    ...
    >
              ...
              >
                ...
                {asset.processed && (
                  <div className="absolute top-1.5 left-1.5">
                    <span className="bg-black/50 text-emerald-400 text-[9px] font-mono px-1 py-0.5 rounded">
                      ✓ ai
                    </span>
                  </div>
                )}
                {asset.destinations.length > 0 && (
                  <div className="absolute top-1.5 right-1.5 flex flex-col gap-0.5 items-end">
                    {asset.destinations.slice(0, 2).map((d) => (
                      <span
                        key={d}
                        className="bg-black/50 text-white text-[9px] font-mono px-1 py-0.5 rounded capitalize"
                      >
                        {d}
                      </span>
                    ))}
                    {asset.destinations.length > 2 && (
                      <span className="bg-black/50 text-white text-[9px] font-mono px-1 py-0.5 rounded">
                        +{asset.destinations.length - 2}
                      </span>
                    )}
                  </div>
      ...
      {selected && (
        <div className="w-72 shrink-0 space-y-4 border-l border-zinc-200 pl-6">
          ...
          {/* AI analysis */}
          {selected.processed &&
            (selected.focal_point || selected.crop_hint) && (
              <div className="space-y-0.5 text-[11px] font-mono text-zinc-400 border-t border-zinc-100 pt-3">
                ...
                {selected.focal_point && (
                  <p>
                    focal: ({selected.focal_point.x.toFixed(2)},{" "}
                    {selected.focal_point.y.toFixed(2)})
                  </p>
                )}
                {selected.crop_hint && (
                  <p>
                    crop: {selected.crop_hint.x.toFixed(2)},
                    {selected.crop_hint.y.toFixed(2)}{" "}
                    {selected.crop_hint.width.toFixed(2)}×
                    {selected.crop_hint.height.toFixed(2)}
                  </p>
                )}
              </div>
            )}
          ...
          <div className="space-y-0.5 text-[11px] font-mono text-zinc-400">
            ...
            <p>
              {selected.mime_type}
              {selected.size_bytes
                ? ` · ${Math.round(selected.size_bytes / 1024)} KB`
                : ""}
            </p>
            <p>
              {new Date(selected.created_at).toLocaleDateString("en-SG", {
                dateStyle: "medium",
              })}
            </p>
          </div>
